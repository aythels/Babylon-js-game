...
//animations (hit, recoil, breath, bob)
...
}; //remov all other animations //reset to initial state
...
this.animationHeadBob = function () {
  ...
  /*
  this.firstCamera.position.addInPlace(new BABYLON.Vector3(
    this.firstCamera.position.x, //Math.cos(this.coumter) * xInt,
    Math.sin(this.counter) * yInt, this.firstCamera.position.z));
*/

  //console.log(Math.sin(this.counter) * yInt);
  scene.onBeforeRenderObservable.remove(observer);

  var observer = scene.onBeforeRenderObservable.add(function () {
    _this.firstCamera.position.y = (Math.sin(_this.counter / 40) * yInt);
...
INPUTMANAGER.addKeyEventInfo('mouseShoot', () => {
  return ['cameraDir', _this.getCameraForwardVec()];
});

INPUTMANAGER.addKeyEventInfo('keysForward', () => {
  return ['cameraDir', _this.getCameraForwardVec()];
});

INPUTMANAGER.addKeyEventInfo('keysBackward', () => {
  return ['cameraDir', _this.getCameraForwardVec()];
});

INPUTMANAGER.addKeyEventInfo('keysRight', () => {
  return ['cameraDir', _this.getCameraForwardVec()];
});

INPUTMANAGER.addKeyEventInfo('keysLeft', () => {
  return ['cameraDir', _this.getCameraForwardVec()];
});

INPUTMANAGER.moveEvent = _this.getCameraForwardVec;

};

/*  INPUTMANAGER.add('test2', () => {
    //cameraManager.animationHeadBob();

    //let forward = cameraManager.getCameraForwardVec();
    //forward.y = 0;
  //  player.playerMesh.physicsImpostor.setLinearVelocity(forward.normalize().scale(300));
    //cameraManager.activateFollowCamera();

    //let skeleton = player.renders[1].skeleton;
    //let animations = scene.getAllAnimatablesByTarget(skeleton);
    //let frame = animations.find(x => x.masterFrame != 0).masterFrame;

    //var mat = new BABYLON.StandardMaterial("Mat", scene);
    //mat.diffuseColor = new BABYLON.Color3(56 / 255, 150 / 255, 255 / 255);
    //mat.emissiveColor = new BABYLON.Color3(0.3, 0.3, 0.3);
    //mat.alpha = 0.3;

    //testlaunch.spawnPlayer(player.playerMesh.feet.position, player.playerMesh.feet.rotation, frame, mat);

  }, 1);*/

  /*INPUTMANAGER.add('mouseAim', () => {
    cameraManager.animationAimCamera();
  }, 1);

  INPUTMANAGER.add('mouseAim', () => {
    cameraManager.animationRetractCamera();
  }, 2);*/
